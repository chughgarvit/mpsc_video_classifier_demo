...
import sys
sys.path.append("/home/zahid/Tset_DEMO_Ready/")
...
def index():
    ...
    videos = ['video0.webm', 'video1.webm', 'video2.webm']  # Example list of videos
...
def predict(classify_type):
    ...
    # values = float(data['segment'])
    # segment_number = int(values)*1800+round((values%1)*3000)  
...
def main(test_dat, pos=0, classify_type="single"):
    ...
    # vid_seg = test_dat.get_data_test(pos)
    with open(str(pos)+'.pickle', 'rb') as f:
...
def process_input():
    data = request.json
    test_dat = data.get('test_dat')
    pos = int(data.get('pos', 0))
    classify_type = data.get('classify_type', 'single')

    # Call your main function here
    videos, messages = main(test_dat, pos, classify_type)

    # Return the messages as a JSON response
    return jsonify({'videos': videos, 'messages': messages})
